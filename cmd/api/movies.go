...
func (app *application) createMovieHandler(w http.ResponseWriter, r *http.Request) {
	...
	// interpolation the system-generated ID for our new movie in the URL.
...
func (app *application) updateMovieHandler(w http.ResponseWriter, r *http.Request) {
	...
	// respones to the client if we couldn't find a matching record.
	movie, err := app.models.Movies.Get(id)
	if err != nil {
		switch {
		case errors.Is(err, data.ErrRecordNotFound):
	...
	// pair was provided in the JSON request body. So we move on and leave the movie record
	// unchanged. Otherwise, we update the movie record with the new title value. Importantly,
	// because input.Title is now a pointer to a string, we need to dereference the pointer
	// using the * operator to get the underlying value before assigning it to our movie record.
	if input.Title != nil {
		movie.Title = *input.Title
	}

	// We also need to do the same for the other fields in the input struct.
	if input.Year != nil {
...
func (app *application) listMoviesHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Extract the sort query string value into the embedded struct, falling back to "id"
	// if it is not provided, by the client (which will imply an ascending sort on movie ID).
